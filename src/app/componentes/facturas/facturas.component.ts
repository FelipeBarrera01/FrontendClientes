...
import { ItemFactura } from 'src/app/interfaces/ItemFactura';
...
export class FacturasComponent implements OnInit {
  ...
  seleccionarProducto(event: MatAutocompleteSelectedEvent): void {
    let produto = event.option.value as Producto;

    if (this.existeItem(produto.id)) {
      this.incrementaCantidad(produto.id);
    } else {
      let nuevoItem: ItemFactura;
      nuevoItem.producto = produto;
